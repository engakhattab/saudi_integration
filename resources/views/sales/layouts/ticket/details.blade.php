...
<table class="myTable" style="width:100%">
    ...
    <tr>
        <td>{{$ticket->ticket_price}} SAR</td>
        @if($ticket->ent_tax != 0)
            <td>{{$ticket->ent_tax}} SAR</td>
        @endif
        @if($ticket->vat != 0)
            <td>{{$ticket->vat}} SAR</td>
        @endif
        <td>{{$ticket->grand_total}} SAR</td>
        <td>{{$ticket->discount_value}} {{($ticket->discount_type == 'per') ? '%' : 'EGP'}}</td>
        <td>{{$ticket->paid_amount}} SAR</td>
        <td>{{$ticket->rem_amount}} SAR</td>
    </tr>
</table>
