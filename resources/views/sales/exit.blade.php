...
@section('content')
    ...
    <div class="card py-4 w-100 w-sm-80 m-auto ">
        <form method="get" action="{{route('exit.index')}}" class="card-body ">
            <label class="form-label fs-4"> <i class="fas fa-ticket-alt me-2"></i>phone , bracelet number or ticket
                number</label>
            <div class="d-flex">
                <input type="text" class="form-control" name="search" value="{{count($models)?$_GET['search']:''}}"
                       id="searchValue" placeholder="Type here...">
    ...
    @if(count($models))
        <div class="card p-2 py-4 mt-3" id="result">
            <div class="screens p-3 row">
                ...
                <div class="screen col">
                    <span>topUp</span>
                    <strong> {{$ticket->total_top_up_price??0}} SAR </strong>
                </div>
                @if($ticket->total_top_down_price != 0)
                    <div class="screen col">
                        <span>Canceled</span>
                        <strong> {{$ticket->total_top_down_price??0}} SAR </strong>
                    </div>
                @endif
            </div>
            ...
            @if($hours)
                <div class=" topUp w-100 w-md-80 m-auto p-3 mb-5 ">
                    <div class="alert alert-primary alert-dismissible fade show text-white bg-gradient-primary"
                         role="alert">
                    ...
                    <div class="paymentMethods">
                        <button class="btn" style="width:17%" type="button" data-bs-toggle="collapse"
                        ...
                        <button class="btn" style="width:22%" type="button" data-bs-toggle="collapse"
                                data-bs-target="#payCash" aria-expanded="false"
                                aria-controls="payCash">
                            <img src="{{asset('assets/sales/img/masterCard.svg')}}" style="width: 35%;">
                            <span> mastercard </span></button>

                        <button class="btn" style="width:25%" type="button" data-bs-toggle="collapse"
                                data-bs-target="#payCash" aria-expanded="false"
                                aria-controls="payCash">
            ...
            <table class=" customDataTable table table-bordered nowrap">
                ...
                @foreach ($models as $model)
                    @php
                        ...
                        if($model->shift_end != null){
                            if ($model->shift_end <= '24:00:00') {
                                $formatted_end = Carbon\Carbon::createFromFormat('H:i:s', $model->shift_end)->format('h:i A'); // Format as 12-hour time if end time is before or equal to 24:00:00
                            } else {
                                $formatted_end = Carbon\Carbon::createFromFormat('H:i:s', $model->shift_end)->subDay()->format('h:i A'); // Format as 12-hour time if end time is after 24:00:00
                            }
                        }
                    @endphp
                    <tr>
                        ...
                        <td>{{$name}}</td>
                        <td>{{$phone}}</td>
                        <td><span class="controlIcons"
                                  id="exitActions{{$model->id}}">{!! $returnArray[$model->id] !!}</span></td>
            ...
            <div class="text-center d-flex justify-content-center">
                <button type="button"
                        ...
                        data-url="{{$type=='rev'?route('reservations.show',$ticket->id):route('ticket.edit',$ticket->id)}}">
                    Reprint
                </button>
                {{--                <button type="button"--}}
                {{--                        class="btn bg-gradient-primary m-3 mb-0" id="print1"--}}
                {{--                        data-url="{{route('printInvoice',$ticket->id) }}">print A4</button>--}}
                <a href="{{route('exit-all',$_GET['search']??'')}}" type="button" class="btn btn-dark m-3 mb-0"> final
                    exit </a>
            </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" id="print"
                                    data-url="{{$type=='rev'?route('reservations.show',$ticket->id):route('ticket.edit',$ticket->id)}}"
                                    class="btn btn-link text-dark ml-auto">No
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <!-- Print1 Modal -->
            <div class="modal fade" id="modal-print1" tabindex="-1" role="dialog" aria-labelledby="modal-print"
                 aria-hidden="true">
                <div class="modal-dialog modal-danger modal-dialog-centered modal-" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-success" data-bs-dismiss="modal">Yes</button>
                            <button type="button" id="print"
                                    data-url="{{route('printInvoice',$ticket->id)}}"
                                    class="btn btn-link text-dark ml-auto">No
                            </button>
                        </div>
                    </div>
